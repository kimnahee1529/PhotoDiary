...
import com.todaylab.cleanarchtemplate.presentation.home.BirthdayUiState
import com.todaylab.cleanarchtemplate.presentation.navigation.AppRoute
import com.todaylab.cleanarchtemplate.presentation.home.WeatherUiState
...
fun MainScreen(
    ...
    birthDateState: BirthdayUiState,
            ...
            TopAppBar(
                title = {
                    Row {
                        ...
                        when {
                            weatherUiState.isLoading -> Text("Loading...")
                            weatherUiState.weather != null -> Text("Weather: ${weatherUiState.weather.icon}")
                            weatherUiState.errorMessage != null -> Text("Error: ${weatherUiState.errorMessage}")
                        }
                    }
                }
            )
        ...
        Column(
            ...
            Row(
                ...
                WheelSpinnerSelector(
                    ...
                    savedText = birthDateState.year,
                    selectedText = year,
                    label = "년",
                    onClick = {
                        spinnerDialogState = SpinnerState(
                            label = "년",
                            items = listOf("", "", * (1900..2024).map { it.toString() }.toTypedArray(), "", ""),
                            onSelected = { year = it }
                        )
                    }
                )
                ...
                WheelSpinnerSelector(
                    ...
                    savedText = birthDateState.month,
                    selectedText = month,
                    label = "월",
                    onClick = {
                        spinnerDialogState = SpinnerState(
                            label = "월",
                            items = listOf("", "", * (1..12).map { it.toString() }.toTypedArray(), "", ""),
                            onSelected = { month = it }
                        )
                    }
                )
                ...
                WheelSpinnerSelector(
                    modifier = Modifier.weight(1f),
                    savedText = birthDateState.day,
                    selectedText = day,
                    label = "일",
                    onClick = {
                        spinnerDialogState = SpinnerState(
                            label = "일",
                            items = listOf("", "", * (1..31).map { it.toString() }.toTypedArray(), "", ""),
                            onSelected = { day = it }
                        )
                    }
                )
